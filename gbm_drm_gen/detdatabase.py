...
class DetDataBase(object):
    ...
    def _det_atscat_data(self):
        ...
        self.lat_cent = np.array(
            [(self.lat_edge[i] + self.lat_edge[i + 1]) / 2.0 for i in range(len(self.lat_edge) - 1)])
        self.theta_cent = np.array(
            [(self.theta_edge[i] + self.theta_edge[i + 1]) / 2.0 for i in range(len(self.theta_edge) - 1)])
        self.phi_cent = 180. - np.array(
            [(self.phi_edge[i] + self.phi_edge[i + 1]) / 2.0 for i in range(len(self.phi_edge) - 1)])
    ...
    def _get_spec_rsp(self, leaf):
        ...
        for fcs, ncs, i in zip(tmp1, tmp2, range(self.ienerg)):
            ...
            for fc, nc in zip(fcs, ncs):
                matrix[i, fc - 1:fc + nc - 1] = rsp['SPECRESP MATRIX'].data["MATRIX"][i][colIndx:colIndx + nc]
                colIndx += nc
