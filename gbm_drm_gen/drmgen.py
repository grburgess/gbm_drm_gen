...
class DRMGen(object):
    ...
    def __init__(self, quaternions, sc_pos, det_number, ebin_edge_in, mat_type=0, ebin_edge_out=None):
        """
    ...
    def to_3ML_response(self, ra, dec):
        ...
        if has_threeml:
            ...
            RuntimeError('3ML is not installed')
    ...
    def to_fits(self, ra, dec, filename, overwrite):
        ...
        response.to_fits("%s_%s.rsp" % (filename, lu[self._det_number]), 'GLAST', 'GBM', overwrite)
    ...
    def _make_drm(self, src_az, src_el, geo_az, geo_el):
        ...
        plat = src_el * sf
        plon = src_az * sf
        P = np.array([np.cos(plat) * np.cos(plon), np.cos(plat) * np.sin(plon), np.sin(plat)], np.float32)

        N = len(self._database.LEND)
        b1, b2, b3, i1, i2, i3 = ftran.trfind(0, P, N, self._database.X, self._database.Y, self._database.Z,
                                              self._database.LIST, self._database.LPTR, self._database.LEND)

        mat1 = self._database.get_rsp(self._database.millizen[i1 - 1], self._database.milliaz[i1 - 1])
        mat2 = self._database.get_rsp(self._database.millizen[i2 - 1], self._database.milliaz[i2 - 1])
        mat3 = self._database.get_rsp(self._database.millizen[i3 - 1], self._database.milliaz[i3 - 1])
        ...
        tmp_phot_bin[1::2] = 10**((np.log10(self._in_edge[:-1]) + np.log10(self._in_edge[1:])) / 2.)
        ...
        if self._matrix_type == 1 or self._matrix_type == 2:
            ...
            if lat <= self._database.lat_edge[-1] and (lat < self._database.lat_cent[-1]):
                ...
                if lat <= self._database.lat_cent[0]:
                    ...
                    il_low = idx - 1
                    il_high = idx
                    l_frac = 1.0 - (lat - self._database.lat_cent[idx]) / (self._database.lat_cent[idx + 1] -
                                                                           self._database.lat_cent[idx])

                # We now have to loop over all the at scat data
                # This could be sped up by moving this to FORTRAN
                tmp_out = np.zeros((len(self._database.theta_cent) * len(self._database.double_phi_cent) * 2,
                                    self._database.ienerg, self._nobins_out))

                tmp_out = at_scat.get_at_scat(gx, gy, gz, il_low, il_high, l_frac, self._nobins_out, self._out_edge,
                                              self._database)

                tmp_out *= coslat_corr
                atscat_diff_matrix = ftran.atscat_highres_ephoton_interpolator(tmp_phot_bin, self._ein, tmp_out)

        ###################################

        new_epx_lo, new_epx_hi, diff_matrix = ftran.highres_ephoton_interpolator(
            tmp_phot_bin, self._ein, out_matrix, self._database.epx_lo, self._database.epx_hi, self._database.ichan,
            n_tmp_phot_bin)

        binned_matrix = ftran.echan_integrator(diff_matrix, new_epx_lo, new_epx_hi, self._database.ichan,
                                               self._out_edge)
        ...
        final_drm[:-1, :] = (
            binned_matrix[::2, :][:-1, :] / 2. + binned_matrix[1::2, :][:-1, :] + binned_matrix[2::2, :] / 2.) / 2.
    ...
    def _at_scat(self, itheta, theta_u, iphi, phi_u, gx, gy, gz, il_low, il_high, l_frac):
        ...
        N = len(self._database.LEND)
        b1, b2, b3, i1, i2, i3 = ftran.trfind(ist, P, N, self._database.X, self._database.Y, self._database.Z,
                                              self._database.LIST, self._database.LPTR, self._database.LEND)
        i_array = np.array([i1, i2, i3])

        dist1 = ftran.calc_sphere_dist(az, 90. - el, self._database.Azimuth[i1 - 1], self._database.Zenith[i1 - 1])
        dist2 = ftran.calc_sphere_dist(az, 90. - el, self._database.Azimuth[i2 - 1], self._database.Zenith[i2 - 1])
        dist3 = ftran.calc_sphere_dist(az, 90. - el, self._database.Azimuth[i3 - 1], self._database.Zenith[i3 - 1])

        dist_array = np.array([dist1, dist2, dist3])
        i1 = i_array[np.argmin(dist_array)]

        tmpdrm = self._database.get_rsp(self._database.millizen[i1 - 1], self._database.milliaz[i1 - 1])

        # intergrate the new drm
        direct_diff_matrix = ftran.echan_integrator(tmpdrm, self._database.epx_lo, self._database.epx_hi,
                                                    self._database.ichan, self._out_edge)

        # Now let FORTRAN add the at scat to the direct
        tmp_out = ftran.sum_at_scat(direct_diff_matrix, self._database.at_scat_data[:, :, il_low, itheta, iphi],
